...
- 行内元素浮动，会改变 display: block。块元素浮动，不设置宽度，宽度会随内容变化
...
静态定位处于标准流中，用于取消定位，top、left、right、bottom 不生效
...
相对定位没有脱离标准流，是相对于自身原有位置进行定位。使用定位后，原有标准流中依然占据位置
...
绝对定位脱离了标准流，定位参照设置了定位的父元素进行定位，如果父元素没有设置定位（设置 static 无效），则参照 html 文档进行定位

```css
position: abslute;
```

> 会改变行内 display 属性 block。会改变块元素，不设置宽度，宽度会随内容变化

#### 固定定位

固定定位脱离了标准流，定位参照浏览器整个显示窗口进行定位，不管页面如何滚动，定位保持不变
...
使用 left/top/right/bottom: 50%，再偏移自身盒子的宽度，即可实现盒子居中。（固定定位也可实现居中，但是相对浏览器整个窗口定位）
...
使用了定位的盒子，后面的盒子会压住前面的盒子，使用 z-index 可调整显示顺序
